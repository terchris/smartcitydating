...
*/
    return companies.map((company, index) => {
      return  <div key={index}>
        <Card>
          <CardContent>
            <Typography variant="headline" component="h2">{company.val.name}</Typography>
            <br />
            <Typography>{company.val.full_name}</Typography>
          </CardContent>
        </Card>
        <Divider inset />
      </div>
  ...
  render () {
    ...
    return (
      <Activity
        ...
        title={intl.formatMessage({ id: 'Companies Card' })}>
        <Scrollbar>
